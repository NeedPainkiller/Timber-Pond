package xyz.needpankiller.timber.pond.service;
...
import xyz.needpankiller.timber.pond.dao.FileEntityRepository;
import xyz.needpankiller.timber.pond.dto.FileInfo;
import xyz.needpankiller.timber.pond.entity.FileAuthorityType;
...
import xyz.needpankiller.timber.pond.helper.FileHelper;
...
//        FileEntity fileEntity = FileEntity.findByUuid(uuid);
        if (fileEntity == null) {
            throw new FileException(FileErrorCode.FILE_DOWNLOAD_FILE_INFO_NOT_EXIST);
        }
        ...
        for (var entry : map.entrySet()) {
            for (FormValue value : entry.getValue()) {
                ...
                if (!isFileItem) {
                    ...
                    throw new FileException(FileErrorCode.FILE_UPLOAD_IS_NOT_MULTIPART);
                }
